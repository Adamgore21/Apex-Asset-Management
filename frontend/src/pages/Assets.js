...
function Assets({ apiUrl, token }) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this asset?')) {
      try {
        await axios.delete(`${apiUrl}/assets/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchAssets();
  ...
  return (
    <div className="page" style={{ display: 'grid', gridTemplateColumns: '2fr 3fr', gap: '2rem' }}>
      {/* Asset List Panel */}
      <div>
        <h1 style={{ margin: '0 0 1.5rem 0', color: '#ff5500', fontSize: '1.6rem', fontWeight: '700' }}>📦 Assets</h1>
            ...
            {assets.map(asset => (
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  background: selectedAsset?.id === asset.id ? '#111111' : 'transparent',
                  borderLeft: selectedAsset?.id === asset.id ? '4px solid #ff5500' : '4px solid transparent',
                  transition: 'all 0.2s'
                }}
                onMouseEnter={(e) => e.currentTarget.style.background = '#111111'}
                onMouseLeave={(e) => e.currentTarget.style.background = selectedAsset?.id === asset.id ? '#111111' : 'transparent'}
              >
        ...
        {selectedAsset ? (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '2rem' }}>
              <div>
                <h2 style={{ margin: '0 0 0.5rem 0', color: '#ff5500', fontSize: '1.4rem', fontWeight: '700' }}>
                  {selectedAsset.name}
                </h2>
                <div style={{ color: '#999', fontSize: '0.9rem' }}>{selectedAsset.asset_id}</div>
              </div>
              <button
            ...
            <div style={{
              ...
              <InfoField label="Location" value={selectedAsset.location} />
...
function InfoField({ label, value, color = '#e5e5e5' }) {
  return (
    <div>
      <div style={{ fontSize: '0.8rem', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>
        {label}
      </div>
